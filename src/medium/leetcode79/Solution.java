...
public class Solution {
    ...
    /**
     * O(n * m * 4word.length())，其中n = board.length，m = board[0].length：
     * dfs：
     * 1、首先遍历整个board数组,每遇到等于word第一个字符时，以这个字符为起点做dfs
     * 2、递归函数要记录一个pos变量，表示当前搜索的深度，或者已经找到的字符串的长度，当pos == word.length -1 的时候，说明已经找到了这个字符串，就可以返回true；
     * 3、四个方向只需要一个即可，遍历每一个位置，从每个位置开始DFS即可，注意边界的判断，用一个visited数组记录已经访问过的位置，防止重复访问（这里在递归的时候使用board[i][j] = '#'，然后递归，就标识了已经访问了，就省去了一个visited数组，当然也可以使用visited数组）
     */
